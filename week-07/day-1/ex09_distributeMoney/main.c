...
{
    srand(time(NULL));
    int cnt_relatives = (rand() % 31) + 20;
    int* relatives = (int*)calloc(cnt_relatives, sizeof(int));

    int money = 1024;
    while (money > 0) {
        int rel_index = rand() % cnt_relatives;
        if (*(relatives + rel_index) == 0) {
            *(relatives + rel_index) = money / 2;
            money /= 2;
        }
    }

    for (int i = 0; i < cnt_relatives; ++i) {
